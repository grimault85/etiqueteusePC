...
//
// Deux fenêtres écriraient tour à tour dans le même fichier de données : la
// dernière à enregistrer effacerait les produits saisis dans l'autre, sans
// aucun avertissement. On refuse donc le second lancement et on ramène la
...
//
// Les produits étaient conservés dans le stockage du navigateur. Celui-ci
// dépend de l'origine et des réglages de Chromium, et s'est révélé peu fiable
// en usage réel : des données disparaissaient d'une session à l'autre.
//
// La version bureau écrit donc dans un simple fichier JSON du dossier
// utilisateur. C'est prévisible, inspectable, et sauvegardable en copiant un
// fichier. La version web continue d'utiliser le stockage du navigateur,
// faute d'alternative.
// ---------------------------------------------------------------------------
const fichierDonnees = () => path.join(app.getPath('userData'), 'donnees.json');

function lireDonnees() {
  try {
    return JSON.parse(fs.readFileSync(fichierDonnees(), 'utf8'));
  } catch (err) {
    return {};   // première utilisation, ou fichier illisible
  }
}

function ecrireDonnees(donnees) {
  const cible = fichierDonnees();
  // Écriture par fichier temporaire puis renommage : une coupure en cours
  // d'écriture ne peut pas laisser un fichier tronqué.
  const temporaire = cible + '.tmp';
  fs.writeFileSync(temporaire, JSON.stringify(donnees, null, 2), 'utf8');
  fs.renameSync(temporaire, cible);
}

// Lecture synchrone : l'interface en a besoin avant son premier affichage.
ipcMain.on('donnees:lire', (evt) => {
  evt.returnValue = lireDonnees();
});

ipcMain.handle('donnees:ecrire', (_evt, donnees) => {
  try {
    ecrireDonnees(donnees);
    return { ok: true, chemin: fichierDonnees() };
  } catch (err) {
    return { ok: false, erreur: String(err && err.message || err) };
  }
});

ipcMain.handle('donnees:chemin', () => fichierDonnees());
...
app.whenReady().then(() => {
  // Sans le verrou, l'application est déjà en cours de fermeture.
